    ...
    <div style={S.overlay} onClick={e=>{if(e.target===e.currentTarget)onClose();}}>
      <div style={S.modal}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          <h3 style={{margin:0,color:"#b5476a",fontSize:16,fontWeight:800}}>⏰ Log Working Hours</h3>
...
function QuickLogPayment({newPayment,setNewPayment,addPayment,onClose}){
  return(
    <div style={S.overlay} onClick={e=>{if(e.target===e.currentTarget)onClose();}}>
      <div style={S.modal}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          <h3 style={{margin:0,color:"#6b48d4",fontSize:16,fontWeight:800}}>💰 Register Payment</h3>
          <button style={S.closeBtn} onClick={onClose}>✕</button>
        </div>
        <div style={S.formGrid}>
        ...
        <button style={{...S.primaryBtn,background:"linear-gradient(135deg,#c4b0f5,#a8d4f5)"}} onClick={addPayment}>Add Payment 🐾</button>
      </div>
    </div>
  );
}
    ...
    <div style={S.list}>
      {items.map(item=>{
        ...
        if(item._type==="session"){
          ...
          card=(
            <div key={item.id+"s"} style={{...S.listItem,borderLeft:"3px solid #f4a7bb"}}>
              <div style={{...S.listLeft,gap:1}}>
                <div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:10,background:"#fce7f0",color:"#b5476a",borderRadius:6,padding:"1px 7px",fontWeight:700,letterSpacing:.3}}>⏰ Hours</span><span style={S.listDate}>{fmtDate(new Date(item.date))}</span></div>
          ...
          card=(
            <div key={item.id+"a"} style={{...S.listItem,borderLeft:"3px solid #a8d4f5"}}>
              <div style={{...S.listLeft,gap:1}}>
                <div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:10,background:"#e8f4ff",color:"#2a5c8a",borderRadius:6,padding:"1px 7px",fontWeight:700,letterSpacing:.3}}>✈️ Airport</span><span style={S.listDate}>{fmtDate(new Date(item.date))}</span></div>
          ...
          card=(
            <div key={item.id+"p"} style={{...S.listItem,background:"#f5f2ff",borderLeft:"3px solid #c4b0f5"}}>
              <div style={{...S.listLeft,gap:1}}>
                <div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:10,background:"#ede8ff",color:"#6b48d4",borderRadius:6,padding:"1px 7px",fontWeight:700,letterSpacing:.3}}>💰 Payment</span><span style={S.listDate}>{fmtDate(new Date(item.date))}</span></div>
  ...
  return(
    <div>
      ...
      <Sect title={`Activities (${activities.length})`}>
        ...
        <div style={S.list}>
          {allDesc.map(a=>(
            <div key={a.id} style={{...S.listItem,background:a.excludeFromOwed?"#fff8f0":"white",border:`1.5px solid ${a.excludeFromOwed?"#ffe0b0":"#e8e0ff"}`,borderLeft:`3px solid ${a.excludeFromOwed?"#f0a830":"#a78bfa"}`}}>
              <div style={S.listLeft}>
                <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                  <span style={{fontSize:10,background:a.excludeFromOwed?"#fff0d0":"#f0ecff",color:a.excludeFromOwed?"#b87a30":"#6b48d4",borderRadius:6,padding:"1px 7px",fontWeight:700}}>🎪 Activity</span>
              ...
              <div style={S.listRight}>
                <span style={{...S.listAmt,color:a.excludeFromOwed?"#b87a30":"#a78bfa"}}>{fmtEuro(a.amount)}</span>
  ...
  return(
    <div>
      <div style={{...S.card,background:"linear-gradient(135deg,#fff0f5 0%,#f0f8ff 100%)",border:"1.5px solid #f9c8d4"}}>
        <h2 style={{...S.cardTitle,color:"#b5476a",marginBottom:4}}>Earnings Stats 📊</h2>
...
function EditPaymentModal({payment,onSave,onClose}){
  ...
  return(<div style={S.overlay}><div style={S.modal}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}><h3 style={{margin:0,color:"#6b48d4",fontSize:16,fontWeight:800}}>✏️ Edit Payment</h3><button style={S.closeBtn} onClick={onClose}>✕</button></div><div style={S.formGrid}><Field label="📅 Date"><input style={S.input} type="date" value={form.date} onChange={e=>setForm(p=>({...p,date:e.target.value}))}/></Field><Field label="💶 Amount"><input style={S.input} type="number" min="0" step="0.01" value={form.amount} onChange={e=>setForm(p=>({...p,amount:+e.target.value}))}/></Field></div><div style={{display:"flex",gap:8,marginTop:8}}><button style={{...S.primaryBtn,background:"#e8f5f0",color:"#3a8a6a",flex:1}} onClick={onClose}>Cancel</button><button style={{...S.primaryBtn,flex:2,background:"linear-gradient(135deg,#c4b0f5,#a8d4f5)"}} onClick={()=>onSave(form)}>Save 🐾</button></div></div></div>);
}
...
const S={
  ...
  listItem:{background:"white",borderRadius:12,padding:"12px 12px",display:"flex",justifyContent:"space-between",alignItems:"center",border:"1px solid #fce7f0"},
  listLeft:{display:"flex",flexDirection:"column",gap:2,flex:1,minWidth:0},
  listDate:{fontSize:13,fontWeight:700,color:"#3a2a35"},
